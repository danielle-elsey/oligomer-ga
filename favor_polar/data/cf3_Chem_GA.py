...
def find_poly_mw(population, smiles_list, end_groups):
    '''
    ...
    for polymer in population:
        ...
        poly_smiles = utils.make_polymer_str(polymer, smiles_list, end_groups)
...
def run_geo_opt(polymer, smiles_list, end_groups):
    '''
    ...
    file_name = utils.make_file_name(polymer)
...
def find_elec_prop(population, smiles_list, end_groups):
    '''
    ...
    #nproc = 8
    for polymer in population:
        run_geo_opt(polymer, smiles_list, end_groups)
...
def fitness_fn(opt_property, poly_property_list, population, poly_size, dip_coeff, pol_coeff):

    if opt_property in ('mw', 'dip', 'pol'):
        ranked_indicies = scoring.simple_descending(poly_property_list)
    elif opt_property == 'dip_pol':
        ranked_indicies = scoring.comb_dip_pol(poly_property_list[0], poly_property_list[1], dip_coeff, pol_coeff, population, poly_size)
    else:
        print('Error: opt_property not recognized; Traceback: fitnesss_fn')
    return(ranked_indicies)


def parent_select(opt_property, population, poly_property_list, poly_size, dip_coeff, pol_coeff):
    '''
    ...
    dip_coeff: float
        weighting coefficient for dipole term in Debye equation
    pol_coeff: float
        weighting coefficient for polarizability term in Debye equation
...
#         fitness_list = fitness_fn_multi('dip', poly_property_list[0], 'pol', poly_property_list[1])
    ...
    fitness_list = fitness_fn(opt_property, poly_property_list, population, poly_size, dip_coeff, pol_coeff)
...
def crossover_mutate(parent_list, pop_size, num_mono_species, sequence_list, smiles_list, end_groups, mono_list):
    '''
    ...
    for parent in new_pop:
        parent_str = utils.make_polymer_str(parent, smiles_list, end_groups)
    ...
    while len(new_pop) < pop_size:
        ...
        temp_child_str = utils.make_polymer_str(
            temp_child, smiles_list, end_groups)
...
def init_gen(pop_size, poly_size, num_mono_species, opt_property, perc, smiles_list, end_groups, dip_coeff, pol_coeff):
    '''
    ...
    params: list (specific format)
        [pop_size, poly_size, num_mono_species, opt_property, smiles_list, end_groups, sequence_list, mono_list, population, poly_property_list, n, gen_counter, spear_counter, prop_value_counter, dip_coeff, pol_coeff]
    '''

    # create all possible numerical sequences for given number of monomer types
    sequence_list = utils.find_sequences(num_mono_species, poly_size)
    ...
    while counter < pop_size:
        ...
        temp_poly_str = utils.make_polymer_str(
            temp_poly, smiles_list, end_groups)
    ...
    setup = subprocess.call('mkdir input output opt failed', shell=True)
            
    # find initial population properties
    if opt_property == 'mw':
        # calculate polymer molecular weights
        poly_property_list = find_poly_mw(population, poly_size, smiles_list, end_groups)
    elif opt_property == 'dip':
        # initialize list of polarizabilities
        polar_list = []
        # calculate electronic properties for each polymer
        elec_prop_list = find_elec_prop(population, smiles_list, end_groups)
    ...
    if opt_property != 'dip_pol':
        ...
        fitness_list = fitness_fn('dip_pol', poly_property_list, population, poly_size, dip_coeff, pol_coeff)
    ...
    for x in range(pop_size):
        ...
        file_name = utils.make_file_name(poly)
    ...
    params = [pop_size, poly_size, num_mono_species, opt_property, smiles_list, end_groups, sequence_list,
              mono_list, population, poly_property_list, n, gen_counter, spear_counter, prop_value_counter, dip_coeff, pol_coeff]
    return(params)
...
def next_gen(params):
    ...
    end_groups = params[5]
    sequence_list = params[6]
    mono_list = params[7]
    population = params[8]
    poly_property_list = params[9]
    n = params[10]
    gen_counter = params[11]
    spear_counter = params[12]
    prop_value_counter = params[13]
    dip_coeff = params[14]
    pol_coeff = params[15]
    ...
    population = parent_select(
        opt_property, population, poly_property_list, poly_size, dip_coeff, pol_coeff)

    # Crossover & Mutation - create children to repopulate bottom 50% of polymers in population
    population = crossover_mutate(
        population, pop_size, num_mono_species, sequence_list, smiles_list, end_groups, mono_list)
    ...
    if opt_property == "mw":
        poly_property_list = find_poly_mw(
            population, poly_size, smiles_list, end_groups)
    elif opt_property == "dip":
        elec_prop_list = find_elec_prop(population, smiles_list, end_groups)
        poly_property_list = elec_prop_list[0]
        polar_list = elec_prop_list[1]
...
def main():
    ...
    # end groups SMILES to use for all polymers in the run
    end_groups = ['CN(C)', 'C(F)(F)F']
    
    # coefficients for weighting dipole and polarizability terms in Debye equation (1=no weighting)
    dip_coeff = (0.389/111)**2
    pol_coeff = 1
    ...
    if restart == 'y':
        ...
        params = init_gen(pop_size, poly_size, num_mono_species, opt_property, perc, smiles_list, end_groups, dip_coeff, pol_coeff)
